			 ...
			 ELSE IF EXISTS (SELECT * FROM parq.tbCargos WHERE carg_Nombre = @carg_Nombre AND carg_Estado  = 0)
				 BEGIN
				   UPDATE parq.tbCargos
					...
					SELECT 200 AS codeStatus, 'Cargo creado con ï¿½xito' AS messageStatus
				 END
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM parq.tbCargos WHERE carg_Nombre = @carg_Nombre AND carg_Estado  = 0 AND carg_ID != @carg_ID)
			BEGIN
				...
				SELECT 200 AS codeStatus, 'Cargo actualizado con ï¿½xito' AS messageStatus
			END
				...
				ELSE
					BEGIN
						UPDATE parq.tbCargos
							...
							carg_Estado		=	0
							WHERE carg_ID	=	@carg_Id
							SELECT 200 AS codeStatus, 'Cargo eliminado con ï¿½xito' AS messageStatus
					END
		COMMIT
	END TRY
	BEGIN CATCH
	...
	BEGIN TRY
		BEGIN TRAN
			IF EXISTS (SELECT * FROM parq.tbRegiones WHERE regi_Nombre = @regi_Nombre AND regi_Estado  = 1 )
				 BEGIN
					SELECT 409 AS codeStatus, 'El nombre de la regiï¿½n ya existe' AS messageStatus
				 END
			 ELSE IF EXISTS (SELECT * FROM parq.tbRegiones WHERE regi_Nombre = @regi_Nombre AND regi_Estado  = 0)
				 BEGIN
				   UPDATE parq.tbRegiones
					...
					SELECT 200 AS codeStatus, 'Regiï¿½n creada con ï¿½xito' AS messageStatus
				 END
...
BEGIN
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM parq.tbRegiones WHERE regi_Nombre = @regi_Nombre AND regi_Estado = 1 AND regi_ID != @regi_ID)
			 BEGIN
				SELECT 409 AS codeStatus, 'El nombre de la regiï¿½n que quieres actualizar ya existe en otro registro.' AS messageStatus
			 END
		IF EXISTS (SELECT * FROM parq.tbRegiones WHERE regi_Nombre = @regi_Nombre AND regi_Estado = 1 AND regi_ID != @regi_ID)
			BEGIN
				UPDATE parq.tbRegiones
				...
				SELECT 200 AS codeStatus, 'Regiï¿½n actualizada con ï¿½xito' AS messageStatus
			END
		ELSE IF NOT EXISTS (SELECT * FROM parq.tbRegiones WHERE regi_Nombre = @regi_Nombre)
				BEGIN
					UPDATE parq.tbRegiones 
	...
	BEGIN TRY
		BEGIN TRAN 
			...
			DECLARE @RegionesEnUso INT = @QuioscosOcupa + @AtraccionesOcupa + @AreasOcupa
				...
				IF	@RegionesEnUso > 0
					BEGIN
						SELECT 500 AS codeStatus, 'La regiï¿½n que desea eliminar estï¿½ en uso' AS messageStatus
					END
				ELSE
					BEGIN
						UPDATE parq.tbRegiones
							...
							regi_Estado		=	0
							WHERE regi_ID	=	@regi_ID
							SELECT 200 AS codeStatus, 'Regiï¿½n eliminada con ï¿½xito' AS messageStatus
					END
		COMMIT
	END TRY
	BEGIN CATCH
	...
	BEGIN TRY
		BEGIN TRAN
			IF EXISTS (SELECT * FROM parq.tbClientes WHERE clie_DNI = @clie_DNI)
			 BEGIN
				SELECT 409 AS codeStatus, 'El DNI que digitï¿½ ya se encuentra registrado.' AS messageStatus
			 END
			 ...
			 BEGIN
				...
				SELECT 200 AS codeStatus, 'Cliente registrado con ï¿½xito' AS messageStatus
			 END
		COMMIT
	END TRY
	BEGIN CATCH
	...
	BEGIN TRY
		...
		ELSE IF NOT EXISTS (SELECT * FROM parq.tbClientes WHERE clie_DNI = @clie_DNI AND clie_ID != @clie_ID)
				BEGIN
					...
					SELECT 200 AS codeStatus, 'Cliente actualizado con ï¿½xito' AS messageStatus
				END
		COMMIT
	END TRY
	BEGIN CATCH
	...
	BEGIN TRY
		BEGIN TRAN 
			...
			DECLARE @EnUso INT = @TicketsClientesOcupa + @ClientesRegistradosOcupa + @RatingsOcupa + @VentasQuioscoOcupa
				...
				IF	@EnUso > 0
					BEGIN
						SELECT 500 AS codeStatus, 'El Cliente que desea eliminar estï¿½ en uso' AS messageStatus
					END
				ELSE
					BEGIN
						UPDATE parq.tbClientes
							...
							clie_Estado		=	0
							WHERE clie_ID	=	@clie_ID
							SELECT 200 AS codeStatus, 'Cliente eliminado con ï¿½xito' AS messageStatus
					END
		COMMIT
	END TRY
	BEGIN CATCH
		...
		BEGIN TRAN
			...
			END
			 ...
			 BEGIN
				...
				SELECT 200 AS codeStatus, 'El usuario fue registrado con Ã©xito' AS messageStatus
			 END
			 ELSE
				BEGIN
		...
		BEGIN TRAN
			...
			IF EXISTS (SELECT * FROM parq.tbClientesRegistrados WHERE clre_Usuario = @clre_Usuario AND clre_ID != @clre_ID)
			 ...
			 BEGIN
				...
				SELECT 200 AS codeStatus, 'El usuario fue actualizado con ï¿½xito' AS messageStatus
			 END
			 ELSE
				BEGIN
					SELECT 409 AS codeStatus, @MessageStatus as messageStatus
...
BEGIN
	BEGIN TRY
		BEGIN TRAN 
				...
				SET		clre_Estado	=	0
				WHERE   clre_ID	= @clre_ID
				SELECT 200 AS codeStatus, 'Cliente eliminado con Ã©xito' AS messageStatus	
		COMMIT
	END TRY
	BEGIN CATCH
		ROLLBACK
			 ...
			 ELSE IF EXISTS (SELECT * FROM parq.tbAreas WHERE area_Nombre = @area_Nombre AND area_Estado  = 0)
				 BEGIN
				   UPDATE parq.tbAreas
					...
					SELECT 200 AS codeStatus, 'Area creada con ï¿½xito' AS messageStatus
				 END
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM parq.tbAreas WHERE area_Nombre = @area_Nombre AND area_Estado  = 0 AND area_ID != @area_ID)
			BEGIN
				...
				SELECT 200 AS codeStatus, 'Area actualizada con ï¿½xito' AS messageStatus
			END
...
BEGIN
	BEGIN TRY
		BEGIN TRAN 
				...
				IF	@RegionesEnUso > 0
					BEGIN
						SELECT 500 AS codeStatus, 'El area que desea eliminar estï¿½ en uso.' AS messageStatus
					END
				ELSE
					BEGIN
						UPDATE parq.tbAreas
							SET area_Estado	= 0 WHERE area_Estado =	@area_ID
						SELECT 200 AS codeStatus, 'Area eliminada con ï¿½xito' AS messageStatus
					END
		COMMIT
	END TRY
	BEGIN CATCH
			 ...
			 ELSE IF EXISTS (SELECT * FROM parq.tbTickets WHERE tckt_Nombre = @tckt_Nombre AND tckt_Estado  = 0)
				 BEGIN
				   UPDATE parq.tbTickets
					...
					SELECT 200 AS codeStatus, 'Ticket creado con ï¿½xito' AS messageStatus
				 END
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM parq.tbTickets WHERE tckt_Nombre = @tckt_Nombre AND tckt_Estado  = 0 AND tckt_ID != @tckt_ID)
			BEGIN
				...
				SELECT 200 AS codeStatus, 'Ticket actualizado con ï¿½xito' AS messageStatus
			END
...
BEGIN
	BEGIN TRY
		BEGIN TRAN 
				...
				IF	@ClienteOcupa > 0
					BEGIN
						SELECT 500 AS codeStatus, 'El Ticket que desea eliminar estï¿½ en uso.' AS messageStatus
					END
				ELSE
					BEGIN
						UPDATE parq.tbTickets
							SET tckt_Estado	= 0 WHERE tckt_ID =	@tckt_ID
						SELECT 200 AS codeStatus, 'Ticket eliminado con ï¿½xito' AS messageStatus
					END
		COMMIT
	END TRY
	BEGIN CATCH
...
BEGIN
	BEGIN TRY
		BEGIN TRAN
					...
					SELECT 200 AS codeStatus, 'Ticket Detalle creado con ï¿½xito' AS messageStatus
		COMMIT
	END TRY
	BEGIN CATCH
		ROLLBACK
...
BEGIN
	BEGIN TRY
				...
				SELECT 200 AS codeStatus, 'Ticket Detalle actualizado con ï¿½xito' AS messageStatus
		COMMIT
	END TRY
	BEGIN CATCH
		ROLLBACK
	...
	BEGIN TRY
		BEGIN TRAN 
				...
				IF	@TicketsClientesOcupa > 0
					BEGIN
						SELECT 500 AS codeStatus, 'El Ticket Detalle que desea eliminar estï¿½ en uso.' AS messageStatus
					END
				ELSE
					BEGIN
