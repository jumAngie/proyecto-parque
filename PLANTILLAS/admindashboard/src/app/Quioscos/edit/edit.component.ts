...
export class EditQuioscoComponent implements OnInit {
  ...
  getData(){
    const id: number | undefined = isNaN(parseInt(localStorage.getItem('quio_ID') ?? '', 0))
    ? undefined : parseInt(localStorage.getItem('quio_ID') ?? '', 0);
  ...
  selectCard(){
    ...
    const currentCard = this.areasForStyle.find(carta => carta.area_ID === this.quiosco.area_ID.toString());
    if(currentCard){
