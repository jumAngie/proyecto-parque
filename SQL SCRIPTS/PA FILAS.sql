...
BEGIN
	BEGIN TRY
		...
		BEGIN
			...
			SELECT 409 AS codeStatus, 'Actualmente este Ticket ya ha sido registrado en una fila de la atracci�n: ' + @atraccion AS messageStatus;
		END
		ELSE
		BEGIN
			...
			SELECT 200 AS codeStatus, 'Posici�n Insertada con �xito' AS messageStatus;
		END
  ...
  AS
	BEGIN
		BEGIN TRY
			...
			SELECT 200 AS codeStatus, 'Temporizador Eliminado con �xito' AS messageStatus;
		END TRY
		BEGIN CATCH
			SELECT 500 AS codeStatus, ERROR_MESSAGE () AS messageStatus
		END CATCH
